#!/usr/bin/env bash
# deploy-addsite.sh v78.1 — auth (login + 2FA optionnel), lignes grisees, modale qui suit le theme,
...
# MODE D'INSTALLATION
# ----------------------------------------------------------------------
# "local" (defaut) : installe DIRECTEMENT sur la machine courante (VM,
#                    bare-metal, conteneur, LXC ou tu as un shell...).
#                    A lancer SUR la machine qui heberge tracker-autovisit.
# "lxc"            : pousse vers un conteneur LXC DEPUIS un hote Proxmox
#                    (via pct). Necessite CT=<CTID>.
#
# Surcharge a l'appel :
#   ./deploy-addsite.sh                 # mode local (defaut)
#   MODE=lxc CT=100 ./deploy-addsite.sh # mode Proxmox/LXC
# ======================================================================
MODE="${MODE:-local}"
CT="${CT:-100}"   # CTID (mode lxc uniquement)

# run <cmd...> : execute une commande sur la cible
run() {
  if [ "$MODE" = "lxc" ]; then pct exec "$CT" -- "$@"; else "$@"; fi
}

# push <src> <dst> [perms] : copie un fichier vers la cible
push() {
  local src="$1" dst="$2" perms="$3"
  if [ "$MODE" = "lxc" ]; then
    if [ -n "$perms" ]; then pct push "$CT" "$src" "$dst" --perms "$perms"
    else pct push "$CT" "$src" "$dst"; fi
  else
    # Mode local : src et dst peuvent etre identiques (ecriture directe en /tmp)
    if [ "$src" != "$dst" ]; then
      install -D ${perms:+-m "$perms"} "$src" "$dst"
    elif [ -n "$perms" ]; then
      chmod "$perms" "$dst"
    fi
  fi
...
echo "[1/8] Pillow + pyotp (favicon + 2FA)…"
run bash -c "pip install pillow pyotp --break-system-packages >/dev/null 2>&1 || true"

echo "[2/8] Dossiers…"
run mkdir -p /opt/tracker-autovisit/data/logs /opt/tracker-autovisit/data/.sitebak /opt/tracker-autovisit/data/cookies /var/www/autovisit/icones /var/www/autovisit/.logos
...
MANEOF
push /tmp/logos-manifest.json /tmp/logos-manifest.json; rm /tmp/logos-manifest.json
run python3 -c 'import json,base64,os
...
FAVPY
push /tmp/fetch_favicons.py /tmp/fetch_favicons.py >/dev/null; rm /tmp/fetch_favicons.py
run python3 /tmp/fetch_favicons.py /tmp/favicon-targets.json || true
run rm -f /tmp/fetch_favicons.py /tmp/favicon-targets.json
...
PYEOF
push /tmp/web-api.py /opt/tracker-autovisit/web-api.py 700; rm /tmp/web-api.py
...
run rm -f /tmp/malinois_patch_2fa.py
...
STATEOF
push /tmp/malinois_patch_stats.py /tmp/malinois_patch_stats.py >/dev/null; rm /tmp/malinois_patch_stats.py
run python3 /tmp/malinois_patch_stats.py || true
run rm -f /tmp/malinois_patch_stats.py
...
UNITEOF
push /tmp/autovisit-web.service /etc/systemd/system/autovisit-web.service; rm /tmp/autovisit-web.service
run systemctl daemon-reload
run systemctl enable autovisit-web >/dev/null 2>&1 || true
...
JSEOF
push /tmp/addsite.js /var/www/autovisit/addsite.js 644; rm /tmp/addsite.js

echo "[7/8] Page index.html…"
run bash -c "[ -f /var/www/autovisit/index.html.orig ] || cp /var/www/autovisit/index.html /var/www/autovisit/index.html.orig 2>/dev/null || true"
...
<script src="/addsite.js?v=78"></script>
</body>
</html>
IDXEOF
push /tmp/index.html /var/www/autovisit/index.html 644; rm /tmp/index.html
run bash -c "chown -R www-data:www-data /var/www/autovisit 2>/dev/null || true"

echo "[8/8] Nginx…"
cat > /tmp/autovisit.nginx << NGEOF
server {
    listen 80;
    server_name ${CT_IP};
    root /var/www/autovisit;
    index index.html;

    location = /addsite.js { add_header Cache-Control "no-store"; }
    location = /index.html { add_header Cache-Control "no-store"; }

    # status.json passe par le service (protege par auth quand activee)
    location = /status.json {
        allow ${LAN_CIDR}; deny all;
...
NGEOF
push /tmp/autovisit.nginx /etc/nginx/sites-available/autovisit; rm /tmp/autovisit.nginx
